...
fclose(datout_fid);
Datout_rtl = (Datout_Re_rtl./2^14) + 1i*(Datout_Im_rtl./2^14);
...
fclose(datout_fid);
IFFT_Mod_rtl = (IFFT_Mod_Re_rtl./2^13) + 1i*(IFFT_Mod_Im_rtl./2^13);
...
for ii = 0:NLOP -1,
    ...
    if (NDS ~=0 )
        for jj = 1:NDS,
            tx_out(:,(PRE + NDS)*ii+2+jj) = tx_d(:,jj);            
        end
    end
end
